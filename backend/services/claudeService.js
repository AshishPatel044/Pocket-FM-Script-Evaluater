...
const { getP0Scripts } = require('./knowledgeBase');
...
When evaluating a script, score these 7 parameters:
...
═══════════════════════════════════════════════
SECTION D — OUTPUT FORMAT (Return this EXACTLY as JSON)
═══════════════════════════════════════════════
...

{
  ...
  "p0Comparison": {
    ...
    "whatP0DoesDifferently": "Specific explanation of what the P0 script does that this one doesn't",
    "keyLessons": [
...
function buildUserPrompt(script, showName, genre, episodeRange, p0Examples) {
  const p0Summary = p0Examples.map(s =>
    `- ${s.id} (${s.show}): Hook pattern: "${s.hookPattern}". Sequence: ${s.sequenceType}. Key strengths: ${s.keyStrengths.slice(0, 3).join('; ')}`
  ).join('\n');

  return `Please evaluate this promo script for the PocketFM show "${showName}" (${genre} genre, episodes ${episodeRange}).
...
Evaluate the script strictly against all the rules and patterns in your system instructions. Be specific — quote actual lines from the script in your feedback. Do not be lenient. A P0 score requires ALL 7 parameters to be strong.

Return ONLY the JSON evaluation. No preamble. No markdown. Pure JSON.`;
}
  ...
  try {
    ...
    if (jsonMatch) {
      try {
        ...
        throw new Error('Failed to parse evaluation response. Please try again.');
      }
    }
...
async function evaluateScript(script, showName, genre, episodeRange) {
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: buildUserPrompt(script, showName, genre, episodeRange, p0Examples)
      }
    ]
  });

  return parseEvaluationResponse(response.content[0].text);
}
